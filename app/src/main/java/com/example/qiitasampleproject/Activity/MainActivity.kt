...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        serchResultListView.setOnItemClickListener { adapterView, view, position, id ->
            this.userPopupShow(firstRepos[position].user)
        }
    }

    @RequiresApi(Build.VERSION_CODES.KITKAT)
    fun userPopupShow(userData: HashMap<String, Object>){
        ...
        val userImageUrl = userData["profile_image_url"]
        Glide.with(this).load(userImageUrl).into(userImageView)

        followeesCountTextView.text = "フォローしている数 :" + userData["followees_count"].toString()
        followersCountTextView.text = "フォローされている数 : " + userData["followers_count"].toString()
        idTextView.text =  "id : " + userData["id"].toString()
    ...
    private fun searchArticle(userName:String){
    ...
    private fun webOpen(url: String){
